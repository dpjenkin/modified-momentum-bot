...
api_key_id = 'AKNBQ4USVZKH6VMW3VTU'
api_secret = '0NBuQsFroH3xUcvYk5RTlZWB9CQeEyBOuldi212u'
...
def get_1000m_history_data(symbols):
    ...
    for symbol in symbols:
        minute_history[symbol] = api.polygon.historic_agg_v2(
            symbol=symbol, multiplier=1, timespan='minute', _from=market_open, to=current_dt,  limit=1000
        ).df
        c += 1
...
def run(tickers, market_open_dt, market_close_dt):
    ...
    for symbol in symbols:
        symbol_channels = ['AM.{}'.format(symbol), 'AM.{}'.format(symbol)]
